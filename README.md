...
### v1.4 — 중복 일정 방지 (2026-07-23)
- 등록 전에 대상 캘린더의 해당 기간 일정을 조회해, **제목·날짜가 같은 일정이 이미 있으면 건너뜀**
- Step 3에 **"이미 등록된 동일 일정은 건너뛰기 (중복 방지)"** 체크박스 추가 (기본 켜짐 — 끄면 항상 새로 등록)
- 결과 집계에 **"중복 건너뜀"** 항목 추가, 로그에 건너뛴 일정 표시
- 같은 표 안에 중복이 있어도 한 번만 등록. 기존 `calendar.events` 권한으로 조회까지 가능(권한 추가 없음)
- 기존 일정 조회 실패 시엔 중복 검사 없이 등록을 계속 진행(로그로 안내)
